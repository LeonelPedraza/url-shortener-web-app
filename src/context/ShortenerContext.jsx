...
export const ShortenerProvider = ({ children }) => {
  ...
  const getAllShortenedsLinks = async () => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:3000/api/get?token=${storage}`
      );
  ...
  const createUserShortenedUrl = async (data) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:3000/api/create?token=${storage}`,
        {
          method: "POST",
          body: jsonData,
          mode: "cors",
  ...
  const updateUserShortenedUrl = async (data, type) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:3000/api/${modalCardActive.id}?token=${storage}`,
        {
          method: "PUT",
          body: bodyData,
          mode: "cors",
  ...
  const deleteUserShortenedUrl = async (id) => {
    try {
      ...
      if (!storage)
        throw new Error("Debe autenticarse para poder acceder a este recurso.");
      const response = await fetch(
        `http://127.0.0.1:3000/api/${id}?token=${storage}`,
        {
          method: "DELETE",
          mode: "cors",
        }
  ...
  const searchUserShortenedUrl = async (query) => {
    try {
      ...
      if (!storage)
        throw new Error("Debe autenticarse para poder acceder a este recurso.");
      const response = await fetch(
        `http://127.0.0.1:3000/api/search?token=${storage}&&search=${query}`
      );
      if (!response.ok)
        throw new Error("A ocurrido un error al procesar la solicitud.");
      const data = await response.json();
